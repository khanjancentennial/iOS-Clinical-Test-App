...
//                 Navigate to home screen after login success
                NavigationLink(destination: HomeScreen(), isActive: $viewModel.patientAddSuccess) {
                    EmptyView()
                }
            }
...
//            .accentColor(.white)// Hide the back button
    ...
    var toastView: some View {
        Group {
            if viewModel.showToast {
                Text("\(viewModel.toastMessage)" ?? "Patient Details Added Successfully")
                    .font(.headline)
                    .foregroundColor(.white)
                    .padding()
...
struct AddPatientScreen_Previews: PreviewProvider {
